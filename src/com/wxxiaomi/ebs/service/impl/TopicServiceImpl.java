...
public class TopicServiceImpl implements TopicService {
	...
	public Result publishTopic(int userid, String content, String pics,
		...
		t.setTime(new Date());
		t.setCcount(0);
		String encode = GeoHashUtil.encode(Double.valueOf(locat_points[0]),
				Double.valueOf(locat_points[1]));
		t.setLocat(encode);
	...
	public Result publishComment(int topicId, String content1, int from_uid,
		...
		try {
			...
			ext.put("action","http://192.168.1.46:8080/ElectricBicycleServer/app/topicList_1.html");
			ext.put("title","收到一条评论");
			CommandMessageBody cmdMsg = new CommandMessageBody(
					MsgTargetType.USERS, new String[] { "122627018" }, "admin",
					ext, "I.m the command message from server");
	...
	// String queryString =
	// "from Topic t where t.userCommonInfo.id=? order by t.id desc";
	// Query queryObject = factory.getCurrentSession()
	// .createQuery(queryString);
	...
	// String queryString =
	// "from Comment c where c.topic_id=? order by c.id desc";
	// Query queryObject = factory.getCurrentSession()
	// .createQuery(queryString);
	...
	// String queryString =
	// "from Comment c where c.to_uid=? order by c.id desc";
	// Query queryObject = factory.getCurrentSession()
	// .createQuery(queryString);
	// queryObject.setParameter(0, userid);
	// return queryObject.list();
	...
	// String queryString =
	// "from Comment c where c.from_uid=? order by c.id desc";
	// Query queryObject = factory.getCurrentSession()
	// .createQuery(queryString);
	// queryObject.setParameter(0, userid);
	// return queryObject.list();
	...
	// // TODO Auto-generated method stub
	// return (Comment) factory.getCurrentSession().get(Comment.class,
	// comment_id);
	// }
	//
	// @SuppressWarnings("unchecked")
	...
	// return
	// factory.getCurrentSession().createQuery("from Topic t where t.id in(:ids)")
	...
	// Object[] array = ids.toArray();
	// return
	// factory.getCurrentSession().createQuery("from Comment t where t.id in(:ids)")
	// .setParameterList("ids", array)
	// .list();
	// }
